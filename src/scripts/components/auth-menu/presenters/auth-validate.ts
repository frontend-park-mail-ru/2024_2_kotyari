...
export class AuthValidate implements validateInterface {
  ...
  validateUsername = (username: HTMLInputElement):any => {
    ...
    if (errorElement)
    this.errorView.removeInputError(username, errorElement);

    if (user.length < 2 || user.length > 40) {
      if (errorElement)
        this.errorView.addInputError(username, errorElement, clientErrors.wrongUsernameLength);
      isValid = false;
  ...
  validatePassword = (password: HTMLInputElement): boolean =>{
    ...
    if (errorMsg) {
      if (errorElement)
        this.errorView.addInputError(password, errorElement, errorMsg);
      return false;
    } else {
      if (errorElement)
        this.errorView.removeInputError(password, errorElement);
      return true;
    }
  }
  ...
  validateEmail = (email:HTMLInputElement):boolean => {
    ...
    if (errorElement)
      this.errorView.removeInputError(email, errorElement);

    if (!emailRegex.test(email.value)) {
      if (errorElement)
        this.errorView.addInputError(email, errorElement, clientErrors.wrongEmailFormat);
  ...
  validatePasswordRepeat = (password: HTMLInputElement, repeatPassword: HTMLInputElement): boolean => {
    ...
    if (repeatPassword.value !== password.value) {
      ...
      if (errorElement)
       this.errorView.addInputError(repeatPassword, errorElement, errorMsg);
      return false;
    } else {
      if (errorElement)
        this.errorView.removeInputError(repeatPassword, errorElement);
      return true;
    }
  };
}